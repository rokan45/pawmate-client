...
const Banner = () => {
  // Fetch available pets count
  const { data: pets = [] } = useQuery({
    queryKey: ["allPetsForBanner"],
    queryFn: async () => {
  ...
  const stats = [
    [availablePets > 0 ? `${availablePets}+` : "...", "Pets Available"],
    [totalStories > 0 ? `${totalStories}+` : "...", "Community Stories"],
    ["50+", "Shelters"],
  ];
      ...
      <div className="max-w-7xl mx-auto px-6 py-20 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        <div className="space-y-6 z-10">
          <div className="badge badge-warning gap-2 px-4 py-3 text-sm font-medium">
            <FaPaw />
            {availablePets > 0 ? `${availablePets}+ Pets Waiting for a Home` : "Pets Waiting for a Home"}
          </div>
          <h1 className="text-5xl lg:text-6xl font-extrabold leading-tight">
            Find Your{" "}
            <span className="text-orange-500">Perfect</span>{" "}
            <span className="text-orange-500">Furry</span> Companion
          </h1>
          <p className="text-lg opacity-70 max-w-md">
            Every pet deserves a loving home. Browse our shelter pets and give
            them the forever home they deserve.
          </p>
          <div className="flex flex-wrap gap-4">
            <Link
              to="/all-pets"
              className="btn bg-orange-500 hover:bg-orange-600 text-white border-none gap-2 px-8 py-3 rounded-full text-lg"
            >
              Adopt Now <FaArrowRight />
            </Link>
            <Link
              to="/all-pets"
              className="btn btn-outline border-orange-500 text-orange-500 hover:bg-orange-500 hover:text-white hover:border-orange-500 rounded-full px-8 py-3 text-lg"
            >
              Browse Pets
            </Link>
          </div>
            ...
            {stats.map(([num, label]) => (
              <div key={label} className="text-center">
                <p className="text-2xl font-bold text-orange-500">
                  {num === "..." ? (
                    <span className="loading loading-dots loading-sm text-orange-400"></span>
                  ) : (
                    num
                  )}
                </p>
